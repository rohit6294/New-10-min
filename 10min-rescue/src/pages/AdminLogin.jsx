...
export default function AdminLogin() {
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      // Only allow users listed in admins/{uid} collection — no hardcoded bypass
      const adminDoc = await getDoc(doc(db, 'admins', cred.user.uid))
      if (adminDoc.exists()) {
        navigate('/admin/dashboard')
      } else {
        await auth.signOut()
        setError('Not authorized as admin. Contact the system owner.')
      }
    } catch (err) {
      const code = err.code
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8">
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-navy mb-1.5">Admin Email</label>
              <input
                type="email"
                required
                value={email}
                onChange={e => setEmail(e.target.value)}
                placeholder="admin@10minrescue.com"
